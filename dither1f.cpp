...
int main(int argc, char **argv)
{
    ...
    int *blueNoise1 = new int[size * size];
    int *blueNoise2 = new int[size * size];
    int *blueNoise3 = new int[size * size];
    unsigned char *image = new unsigned char[nPixels * 3];

    FILE *file;

    file = ::fopen("noise5a.bin", "rb");
    fread(blueNoise1, sizeof(int), size * size, file);
    fclose(file);

    file = ::fopen("noise5b.bin", "rb");
    fread(blueNoise2, sizeof(int), size * size, file);
    fclose(file);

    file = ::fopen("noise5c.bin", "rb");
    ...
    auto drawDither2Rect = [image, width, height, blueNoise1, blueNoise2, blueNoise3, size, sizeMask](int x, int y, int w, int h, float r, float g, float b) {
        const int y2 = y + h;
        const int x2 = x + w;
        ...
        for(int yy = y; yy < y2; yy++)
        {
            ...
            {
                int bn1 = blueNoise1[(yy & sizeMask) * size + (xx & sizeMask)];
                int bn2 = blueNoise2[(yy & sizeMask) * size + (xx & sizeMask)];
                int bn3 = blueNoise3[(yy & sizeMask) * size + (xx & sizeMask)];
    ...
    FILE *file2 = ::fopen("image1f.ppm", "wb");
